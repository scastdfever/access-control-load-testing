...
class CodesValidationSimulationAccessControl : Simulation() {

  // 1. Configuration: Define constants and system properties for flexibility.
  private val partnerId = 198
  private val baseUrl = System.getProperty("host.url", "http://localhost:8020")
          ...
          .on(
              exec(
                  http("POST_validate_code_#{codeMap.code}") // Use the code in the request name
                      // for uniqueness
                      .post("/api/1.0/partners/$partnerId/codes/validate")
                      .body(
                          StringBody(
